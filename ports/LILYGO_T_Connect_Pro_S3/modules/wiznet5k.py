        ...
        while True:
            try:
                ...
                if time.time() - start_time > timeout:
                    print("Failed to initialize WIZnet module within timeout.")
                    return  # 超时返回，确保不卡住
            time.sleep(0.1)
        ret = self.set_dhcp(hostname, 1)
        if ret != 0:
            self._dhcp_client = None
#         assert ret == 0, "Failed to configure DHCP Server!"
    ...
    def link_status(self):
        ...
        if self._chip_type == "w5500":
            ...
            if data[0] & 0x01:  # Check if link is up
                return 1  # Link is ON
            else:
                return 2  # Link is OFF
        return 0  # Or return a default value for unsupported types
